...
export const ORDER_INCLUDABLE_RELATIONS = {
  "user": {
    "profile": {
      "user": {}
    },
...
export const PRODUCT_INCLUDABLE_RELATIONS = {
  "orders": {
    "order": {
      "user": {
        "profile": {
...
export const ProductGetQuerySchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    const includeError = validateIncludePaths(data.include, PRODUCT_INCLUDABLE_RELATIONS);
    if (includeError) {
      ctx.addIssue({
        code: 'custom',
        message: includeError,
...
export const PRODUCT_ORDER_INCLUDABLE_RELATIONS = {
  "order": {
    "user": {
      "profile": {
        "user": {}
...
export const ProductOrderGetQuerySchema = z
  .object({
    include: z.string().optional(),
  })
  .superRefine((data, ctx) => {
    ...
    const includeError = validateIncludePaths(data.include, PRODUCT_ORDER_INCLUDABLE_RELATIONS);
    if (includeError) {
      ctx.addIssue({
        code: 'custom',
        message: includeError,
