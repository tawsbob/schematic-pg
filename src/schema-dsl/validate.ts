...
function validateUniqueConstraintsIncludeKey(
  ...
  for (const directive of model.directives.filter((item) => item.name === 'unique')) {
    ...
    if (!keySetIsCovered(fields, keySet)) {
      throw new SchemaError(
        `unique constraint must include partition key fields [${partitionFields.join(', ')}]`,
        directive.loc,
      );
    }
  }
...
function validateUniqueDirectives(model: Model, modelNames: Set<string>): void {
  const storedNames = new Set(
    model.fields.filter((field) => !modelNames.has(field.type.name)).map((field) => field.name),
  );
  const seenFieldLists = new Map<string, SourceLocation>();
  ...
  for (const directive of model.directives.filter((item) => item.name === 'unique')) {
    ...
    for (const pair of directive.args.pairs) {
      if (!UNIQUE_DIRECTIVE_KEYS.has(pair.key)) {
        throw new SchemaError(
          `@@unique does not support "${pair.key}" (use @@index with unique: true for where/type)`,
          directive.loc,
        );
      }
    }
    ...
    const fields = fieldsPair.value.elements.map((element) => {
      if (element.kind !== 'Identifier') {
        throw new SchemaError(
          `@@unique fields must be identifiers on model "${model.name}"`,
          directive.loc,
        );
      }
      return element.name;
    ...
    if (fields.length < 2) {
      throw new SchemaError(
        `@@unique requires at least two fields (use @unique for a single column)`,
        directive.loc,
      );
    }
    ...
    for (const fieldName of fields) {
      if (!storedNames.has(fieldName)) {
        throw new SchemaError(
          `@@unique field "${fieldName}" is not a stored column on model "${model.name}"`,
          directive.loc,
        );
      }
      if (seenInList.has(fieldName)) {
        throw new SchemaError(
          `@@unique lists field "${fieldName}" more than once on model "${model.name}"`,
          directive.loc,
        );
      }
    ...
    if (existing) {
      throw new SchemaError(
        `duplicate @@unique field list on model "${model.name}"`,
        directive.loc,
      );
    }
...
function validateViews(schema: Schema): void {
  const enumNames = new Set(schema.enums.map((enumDef) => enumDef.name));
  const modelSqlNames = new Map(
    schema.models.map((model) => [toTableName(model.name), model.name]),
  );
  const viewSqlNames = new Map<string, string>();
  ...
  for (const view of schema.views) {
    ...
    if (modelClash) {
      throw new SchemaError(
        `view "${view.name}" SQL name "${sqlName}" conflicts with model "${modelClash}"`,
        view.loc,
      );
    }
    ...
    if (viewClash) {
      throw new SchemaError(
        `view "${view.name}" SQL name "${sqlName}" conflicts with view "${viewClash}"`,
        view.loc,
      );
    }
    ...
    for (const column of view.columns) {
      if (columnNames.has(column.name)) {
        throw new SchemaError(
          `duplicate column "${column.name}" on view "${view.name}"`,
          column.loc,
        );
      }
      ...
      if (!PRIMITIVE_TYPES.has(typeName) && !enumNames.has(typeName)) {
        throw new SchemaError(
          `view column "${view.name}.${column.name}" type must be a primitive or enum, got "${typeName}"`,
          column.type.loc,
        );
      }
      ...
      for (const attribute of column.attributes) {
        if (VIEW_DISALLOWED_FIELD_ATTRS.has(attribute.name)) {
          throw new SchemaError(
            `@${attribute.name} is not allowed on view column "${view.name}.${column.name}"`,
            attribute.loc,
          );
        }
      }
    ...
    for (const directive of view.directives) {
      if (!VIEW_ALLOWED_DIRECTIVES.has(directive.name)) {
        throw new SchemaError(
          `@@${directive.name} is not allowed on view "${view.name}"`,
          directive.loc,
        );
      }
      ...
      if (directive.name === 'index') {
        if (!view.materialized) {
          throw new SchemaError(
            `@@index is only allowed on materialized views (view "${view.name}")`,
            directive.loc,
          );
        }
...
function validateViewIndexFields(
  view: View,
  directive: { args?: { kind: string; pairs?: Array<{ key: string; value: Value }> }; loc: SourceLocation },
  ...
  for (const element of fieldsPair.value.elements) {
    ...
    if (!columnNames.has(element.name)) {
      throw new SchemaError(
        `@@index field "${element.name}" is not a column on view "${view.name}"`,
        directive.loc,
      );
    }
  }
...
function validateViewIdDirective(
  view: View,
  directive: { args?: { kind: string; pairs?: Array<{ key: string; value: Value }> }; loc: SourceLocation },
  columnNames: Set<string>,
): void {
  ...
  for (const element of fieldsPair.value.elements) {
    ...
    if (!columnNames.has(element.name)) {
      throw new SchemaError(
        `@@id field "${element.name}" is not a column on view "${view.name}"`,
        directive.loc,
      );
    }
  }
...
function validateViewRest(view: View): void {
  ...
  for (const operation of operations) {
    if (VIEW_WRITE_OPS.has(operation)) {
      throw new SchemaError(
        `@rest on view "${view.name}" cannot include write operation "${operation}"`,
        rest?.loc ?? view.loc,
      );
    }
    if (!VIEW_READ_OPS.has(operation)) {
      throw new SchemaError(
        `unknown @rest operation "${operation}" on view "${view.name}"`,
        rest?.loc ?? view.loc,
      );
    }
  }
  ...
  if (operations.has('get') && !viewHasPrimaryKey(view)) {
    throw new SchemaError(
      `@rest get on view "${view.name}" requires @id or @@id`,
      rest?.loc ?? view.loc,
    );
  }
}

function viewHasPrimaryKey(view: View): boolean {
...
function parseViewRestOperations(view: View, rest: { args?: import('./ast.js').AttributeArgs; loc: SourceLocation }): Set<string> {
  if (!rest.args) {
    return new Set();
  }
  ...
  if (onlyPair) {
    ...
    return new Set(
      onlyPair.value.elements.map((element) => {
        if (element.kind !== 'Identifier') {
  ...
  const excluded = new Set(
    exceptPair!.value.elements.map((element) => {
      if (element.kind !== 'Identifier') {
        throw new SchemaError(`@rest operation on view "${view.name}" must be an identifier`, rest.loc);
      }
      return element.name.toLowerCase();
...
function validateViewPolicies(view: View): void {
  for (const attribute of view.attributes) {
    ...
    if (allowPair.value.kind === 'Identifier') {
      if (allowPair.value.name !== 'all' && allowPair.value.name !== 'select') {
        throw new SchemaError(
          `@policy on view "${view.name}" allow must be select or all`,
          allowPair.loc ?? attribute.loc,
        );
      }
      continue;
    }
    ...
    if (allowPair.value.kind !== 'ArrayLiteral') {
      throw new SchemaError(
        `@policy on view "${view.name}" allow must be select, all, or an array`,
        allowPair.loc ?? attribute.loc,
      );
    }
    ...
    for (const element of allowPair.value.elements) {
      if (element.kind !== 'Identifier') {
        throw new SchemaError(
          `@policy operation on view "${view.name}" must be an identifier`,
          allowPair.loc ?? attribute.loc,
        );
      }
      if (element.name.toLowerCase() !== 'select') {
        throw new SchemaError(
          `@policy on view "${view.name}" allow may only include select`,
          allowPair.loc ?? attribute.loc,
        );
      }
    }
