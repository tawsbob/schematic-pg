...
export class Parser {
  ...
  private parsePredicatesSection(): Predicate[] {
  ...
  private parsePredicate(existingNames: Set<string>): Predicate {
    const nameToken = this.expect(TokenType.IDENT, 'predicate name');
    if (existingNames.has(nameToken.value)) {
      throw new ParseError(
        `unique predicate name, "${nameToken.value}" already defined`,
        nameToken,
        this.file,
      );
    }
  ...
  private parseCronSection(): CronJob[] {
  ...
  parseCronJob(existingNames?: Set<string>): CronJob {
    ...
    if (existingNames?.has(nameToken.value)) {
      throw new ParseError(
        `unique job name, "${nameToken.value}" already defined`,
        nameToken,
        this.file,
      );
    }
  ...
  private parseCronJobBody(): {
    ...
    do {
      ...
      switch (keyToken.value) {
        ...
        default:
          throw new ParseError(
            "job body key 'schedule', 'execute', or 'call'",
            keyToken,
            this.file,
          );
      }
