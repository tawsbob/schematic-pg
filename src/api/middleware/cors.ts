...
export function parseCorsOrigin(raw = process.env.CORS_ORIGIN): string | string[] | null {
  const trimmed = raw?.trim() ?? '';
  if (!trimmed) {
    return null;
  }
...
export function parseCorsAllowHeaders(raw = process.env.CORS_ALLOW_HEADERS): string[] {
  return parseCommaSeparatedList(raw);
}

function resolveAllowHeaders(optionsAllowHeaders?: string[]): string[] {
