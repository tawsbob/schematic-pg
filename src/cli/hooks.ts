...
function resolveSchemaArg(args: string[]): string | undefined {
...
function getHookFilePath(hooksDir: string, modelName: string): string {
  return path.join(hooksDir, `${modelName}.ts`);
}

function assertModelExists(schema: Schema, modelName: string): Model {
...
function assertHookFileDoesNotExist(hooksDir: string, modelName: string): void {
...
async function promptForModel(models: Model[], hooksDir: string, schema: Schema): Promise<string> {
...
export async function runHooksAdd(args: string[], options: RunHooksAddOptions = {}): Promise<string> {
  ...
  const modelName =
    options.modelName ?? parseModelFlag(args) ?? (await promptForModel(schema.models, hooksDir, schema));
