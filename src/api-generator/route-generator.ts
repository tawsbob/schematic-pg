...
export class RouteGenerator {
  private readonly modelsWithHooks: ReadonlySet<string>;
  private readonly overlays: ReadonlyMap<string, CustomRouteMountEntry>;
  ...
  generate(): string | null {
    ...
    const needsPk =
      operations.has('get') || operations.has('update') || operations.has('delete');
    ...
    const whereFromParams =
      primaryKey?.fields.map((field) => `${field}: params.${field}`).join(', ') ?? '';
    ...
    const hasWriteOps =
      operations.has('create') || operations.has('update') || operations.has('delete');
    ...
    const validateImports: string[] = [];
    if (needsValidateJson) validateImports.push('validateJson');
    if (needsValidateParam) validateImports.push('validateParam');
    if (needsValidateQuery) validateImports.push('validateQuery');

    const schemaImports: string[] = [];
    if (operations.has('create')) schemaImports.push(`  ${this.model.name}CreateSchema,`);
    if (operations.has('update')) schemaImports.push(`  ${this.model.name}UpdateSchema,`);
    if (needsValidateParam) schemaImports.push(`  ${paramSchemaName},`);
    if (operations.has('list')) schemaImports.push(`  ${listQuerySchemaName},`);
    if (operations.has('get')) schemaImports.push(`  ${getQuerySchemaName},`);
    ...
    if (needsOmitFields) schemaImports.push(`  ${constantPrefix}_OMIT_FIELDS,`);
    ...
    if (validateImports.length > 0) {
      lines.push(
        `import { ${validateImports.join(', ')} } from '${PACKAGE_NAME}/api/middleware/validate';`,
      );
    }
    if (needsNotFound) {
  ...
  private generateOverlayOnly(overlay: CustomRouteMountEntry): string {
    return [
      '// Auto-generated by RouteGenerator. Do not edit manually.',
      "import { Hono } from 'hono';",
      `import type { AppEnv } from '${PACKAGE_NAME}/api/types';`,
...
export function generateRouteFiles(
  schema: Schema,
  modelsWithHooksOrOptions: ReadonlySet<string> | RouteGeneratorOptions = new Set(),
): Map<string, string> {
...
function normalizeRouteGeneratorOptions(
  value: ReadonlySet<string> | RouteGeneratorOptions,
  ...
  if (
    ...
    return { modelsWithHooks: value as ReadonlySet<string> };
  }

  return value as RouteGeneratorOptions;
}

export function getRouteMountEntries(
  schema: Schema,
  overlays: ReadonlyMap<string, CustomRouteMountEntry> = new Map(),
): { basePath: string; fileName: string; importName: string }[] {
