...
export class OpenApiGenerator {
  ...
  generate(): OpenApiDocument {
    ...
    return {
      ...
      info: {
        ...
        version: '1.0.0',
        description:
          'Auto-generated OpenAPI document from the schema AST. Regenerated by schematic-pg generate:api.',
      },
      components: {
        securitySchemes: {
          bearerAuth: {
  ...
  private buildModelComponentSchemas(model: Model): Record<string, JsonSchema> {
    const modelNames = getModelNames(this.schema);
    const storedFields = getStoredFields(model, modelNames).filter((field) =>
      isStoredScalarField(field, this.schema),
    );
  ...
  private buildListQueryParameters(model: Model): OpenApiParameter[] {
    ...
    for (const field of filterableFields) {
      ...
      for (const operator of meta.operators) {
        const name = queryParamKey(field.name, operator as FilterOperator);
        parameters.push({
          name,
          in: 'query',
          required: false,
          schema: this.filterParamSchema(field, operator as FilterOperator),
          description:
            operator === 'equals'
  ...
  private buildIncludeParameter(model: Model): OpenApiParameter {
  ...
  private filterParamSchema(field: Field, operator: FilterOperator): JsonSchema {
    if (operator === 'in') {
      return {
        type: 'string',
  ...
  private fieldToOpenApiSchema(
    field: Field,
    options: { nullable: boolean } = { nullable: false },
  ): JsonSchema {
  ...
  private mapTypeToOpenApi(type: TypeExpr): JsonSchema {
  ...
  private buildAuthComponentSchemas(): Record<string, JsonSchema> {
    return {
      AuthRegisterRequest: {
        type: 'object',
      ...
      AuthTokenResponse: {
        ...
        properties: {
          token: {
            type: 'string',
            description:
              'Short-lived access JWT. Keep in memory and send as Authorization: Bearer. Never store in localStorage/sessionStorage.',
          },
          expiresIn: {
            type: 'integer',
  ...
  private buildAuthPaths(): Record<string, OpenApiPathItem> {
    return {
      '/auth/register': {
        post: {
          tags: ['Auth'],
          summary: 'Register',
          description:
            'Creates a user, returns an access JWT in JSON, and sets an HttpOnly refresh_token cookie (Path=/auth). The refresh token is never included in the response body.',
          ...
          responses: {
            '201': {
              description:
                'Registered user with access token. Refresh token is Set-Cookie only (HttpOnly).',
      ...
      '/auth/login': {
        post: {
          ...
          summary: 'Login',
          description:
            'Verifies password, returns an access JWT in JSON, and sets an HttpOnly refresh_token cookie (Path=/auth). The refresh token is never included in the response body.',
          ...
          responses: {
            '200': {
              description:
                'Access token and user. Refresh token is Set-Cookie only (HttpOnly).',
              content: jsonContent({ $ref: '#/components/schemas/AuthTokenResponse' }),
            },
            '400': errorResponse('Validation error', ERROR_EXAMPLE_VALIDATION),
      ...
      '/auth/refresh': {
        post: {
          ...
          description:
            'Reads the HttpOnly refresh_token cookie, rotates the session, returns a new access JWT, and sets a new refresh cookie. Call with credentials: include. Single-flight refreshes on the client to avoid reuse detection.',
      ...
      '/auth/logout': {
        post: {
          ...
          summary: 'Logout',
          description:
            'Revokes the refresh-token family for the current cookie and clears the cookie. Returns 204 even when no cookie was sent.',
...
export function generateOpenApiFiles(
  schema: Schema,
  options?: OpenApiGeneratorOptions,
): { openapiTs: string; openapiJson: string } {
