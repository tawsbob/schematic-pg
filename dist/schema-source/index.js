...
import { mergeFragments, parseFragment, validateMergedSchema, validateSchema, } from '../schema-dsl/index.js';
...
export function resolveSchemaSource(arg, cwd = process.cwd()) {
    if (arg) {
...
export function describeSchemaSource(source) {
    return source.kind === 'file' ? source.path : source.dir;
}
export function loadSchema(source) {
    if (source.kind === 'file') {
    ...
    const fragments = source.files.map((file) => {
...
export function loadSchemaFromArg(arg, cwd = process.cwd()) {
