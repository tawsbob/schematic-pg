...
export class ZodSchemaGenerator {
    ...
    generateViewSchemas(view) {
        ...
        const blocks = [];
        if (paramLines.length > 0) {
            blocks.push(`export const ${view.name}ParamSchema = z.object({`, ...paramLines.map((line) => `  ${line},`), `});\n`);
        }
        blocks.push(...this.generateListQuerySchemas(view));
        return blocks;
    }
    generateListQuerySchemas(relation) {
        ...
        const includableRelationsJson = JSON.stringify(relation.kind === 'Model' ? buildIncludableRelationTree(relation, this.schema) : {}, null, 2);
        ...
        const getQuerySchema = relation.kind === 'Model'
            ? [
    ...
    generateViewReadQueryRefinement(prefix) {
        return [
