import { collectForeignKeys, getDirectives, getEnumNames, getModelNames, getStoredFields, normalizeIndexDirective, normalizeTriggerDirective, resolveTriggerNames, serializeColumnType, } from '../utils/ast-helpers.js';
...
export function generatePartitionConvertSql(kind, modelName, oldSchema, newSchema) {
    ...
    const copySql = formatDataCopy(oldModel, newModel, oldEnumNames, newEnumNames, oldModelNames, newModelNames, quotedTable, quotedStaging);
    if (copySql) {
    ...
    const newTriggerKeys = new Set(getDirectives(newModel, 'trigger').map((directive) => {
        const normalized = normalizeTriggerDirective(directive);
...
function formatDataCopy(oldModel, newModel, oldEnumNames, newEnumNames, oldModelNames, newModelNames, quotedTable, quotedStaging) {
    const oldFields = new Map(getStoredFields(oldModel, oldModelNames).map((field) => [field.name, field]));
    const sharedFields = getStoredFields(newModel, newModelNames).filter((field) => oldFields.has(field.name));
